...
def plot_relative_discretization_error(mean_array, std_array, label, img_series, filter=None):
    # Values for x-axis
    ...
    if filter:
        fig.savefig("../plot_output/" + label + "_" + img_series + filter)
    else:
...
if __name__ == '__main__':
    ...
    image_name_list_noise_a: list = ['rect1a', 'rect2a', 'rect3a', 'rect4a']
    image_name_list_noise_b: list = ['rect1b', 'rect2b', 'rect3b', 'rect4b']
    image_series: dict = {"a": image_name_list_noise_a,
                          "b": image_name_list_noise_b}

    # Initialize parameters for Gaussian filter
    gauss_parameter_list = [2, 3]
    ...
    print("Analysis on images without noise")
    ...
    for serie, images in image_series.items():
        print("Analysis on images serie " + serie)
        ...
        for gauss_parameter_value in gauss_parameter_list:
            ...
            # Now, all the mean and std values are known for images (rect)1-4 that are used for the following:
            # Plot the graph of the relative discretization error of the size
            plot_relative_discretization_error(np.array(mean_size), np.array(std_size), 'size', serie,
                                               '_gauss_' + str(gauss_parameter_value))
            # Plot the graph of the relative discretization error of the perimeter
            plot_relative_discretization_error(np.array(mean_perimeter), np.array(std_perimeter), 'perimeter',
                                               serie, '_gauss_' + str(gauss_parameter_value))



        # for image_name in serie:
        # Apply Gaussian filter with different parameters
        for median_parameter in median_parameter_list:
            ...
            # Plot the graph of the relative discretization error of the size
            plot_relative_discretization_error(np.array(mean_size), np.array(std_size), 'size', serie,
                                               '_median_' + median_parameter)
            # Plot the graph of the relative discretization error of the perimeter
            plot_relative_discretization_error(np.array(mean_perimeter), np.array(std_perimeter), 'perimeter',
                                               serie, '_median_' + median_parameter)
