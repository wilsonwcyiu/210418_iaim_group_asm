...
def segm_for_tracking(img: diplib.Image, mask_img: diplib.Image, image_file_name: str, proj_dir: str, show: bool):
    file_name_to_save: str = image_file_name + '_mask.png'
    CommonUtil.save_image_to_default_project_folder_imageio(mask_img, 'asm4', file_name_to_save, proj_dir)
    ...
    file_name_to_save: str = image_file_name + '_watershed.png'
    CommonUtil.save_image_to_default_project_folder_imageio(watershed_img, 'asm4', file_name_to_save, proj_dir)

    segm_img: diplib.Image = diplib.Invert(watershed_img)

    file_name_to_save: str = image_file_name + '_segm.png'
...
if __name__ == '__main__':
    ...
    for i in range(total_amount_series):
        ...
        for selected_cell in selected_cell_list:
            ...
            CommonUtil.save_image_to_default_project_folder_imageio(selected_cell.cell_track_img, 'asm4',
                                                            image_series_name_list[i] + '_tracking_cell_' + str(
                                                                selected_cell.cell_id) + '.png', proj_dir_path)
