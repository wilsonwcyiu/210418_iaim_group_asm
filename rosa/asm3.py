...
if __name__ == '__main__':
    ...
    for image_name in image_name_list:
        img = UtilFunctions.obtain_image(image_name)
        img = dip.Opening(img)
        UtilFunctions.save_image_to_default_project_folder(img, "asm3", image_name.replace('.tif', '_1_opening.tif'))

        even_background_img = UtilFunctions.black_hat_transf(img)
        UtilFunctions.save_image_to_default_project_folder(even_background_img, "asm3", image_name.replace('.tif', '_2_black_hat.tif'))

        kernel = dip.PyDIP_bin.Kernel(shape='rectangular', param=[30])
        no_noise_img = dip.MedianFilter(even_background_img, kernel)
        UtilFunctions.save_image_to_default_project_folder(no_noise_img, "asm3", image_name.replace('.tif', '_3_median_filter.tif'))

        segm_img = UtilFunctions.segment_image(no_noise_img)

        UtilFunctions.save_image_to_default_project_folder(segm_img, "asm3", image_name.replace('.tif', '_4_segm.tif'))
